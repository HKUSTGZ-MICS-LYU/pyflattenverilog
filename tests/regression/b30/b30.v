...
module add_dec(a_bus, clock, rst, add_decoded, add_decoded_r, add_test_en, a_dig_in, a_dig_out, a_fault_dec, a_fault_dec_r);
   ...
   input                     clock;
   input                     rst;
   output [THIRTEEN_BIT-1:0] add_decoded;
   reg [THIRTEEN_BIT-1:0]    add_decoded;
   ...
   output                    a_dig_out;
   reg                       a_dig_out;
   output [TWO_BIT-1:0]      a_fault_dec;
   reg [TWO_BIT-1:0]         a_fault_dec;
...
module ph2_handle(clock, cur_state, end_on, end_period, i_fbk, rst, sh_mode, global2_state);
   ...
   input                rst;
   input                sh_mode;
   output [TWO_BIT-1:0] global2_state;
   reg [TWO_BIT-1:0]    global2_state;
...
module ph4_handle(clock, cur_state, end_on, end_period, i_fbk, rst, sh_mode, global4_state);
   ...
   input                rst;
   input                sh_mode;
   output [TWO_BIT-1:0] global4_state;
   reg [TWO_BIT-1:0]    global4_state;
...
module sel_global_state(cur_state, global1_state, global2_state, global4_state, global_state);
   ...
   input [TWO_BIT-1:0]          global2_state;
   input [TWO_BIT-1:0]          global4_state;
   output [GLOB_STATE_BITS-1:0] global_state;
   reg [GLOB_STATE_BITS-1:0]    global_state;
...
module split(clock, rst, inj_cmd, i_fbk_f, v_fbk_f, inj_cmd_034, inj_cmd_125, i_fbk_034, i_fbk_125, v_fbk_034, v_fbk_125);
   ...
   input [BANKS_NUMBER-1:0]  i_fbk_f;
   input [INJ_NUMBER-1:0]    v_fbk_f;
   output [SPLIT_NUMBER-1:0] inj_cmd_034;
   reg [SPLIT_NUMBER-1:0]    inj_cmd_034;
   output [SPLIT_NUMBER-1:0] inj_cmd_125;
   reg [SPLIT_NUMBER-1:0]    inj_cmd_125;
   output                    i_fbk_034;
   reg                       i_fbk_034;
   output                    i_fbk_125;
   reg                       i_fbk_125;
   output [SPLIT_NUMBER-1:0] v_fbk_034;
   reg [SPLIT_NUMBER-1:0]    v_fbk_034;
   output [SPLIT_NUMBER-1:0] v_fbk_125;
   reg [SPLIT_NUMBER-1:0]    v_fbk_125;
   reg [5:0]                 inj_cmd_int;
...
module state_progression(clock, cmd_stored, comp, enable_check, end_t0, global_state, inj_cmd, i_fbk, rst, sh_mode, status_reg, t4_0, th_0, v_fbk_cur, v_fbk_mask, cur_state);
   ...
   input                       v_fbk_cur;
   input [SPLIT_NUMBER-1:0]    v_fbk_mask;
   output [FIVE_BIT-1:0]       cur_state;
   reg [FIVE_BIT-1:0]          cur_state;
...
module test_en(add_test_en, clock, d_bus, rd_en, rst, wr_en, test_en_034, test_en_125);
   ...
   input                    wr_en;
   output [THREE_BIT-1:0]   test_en_034;
   reg [THREE_BIT-1:0]      test_en_034;
   output [THREE_BIT-1:0]   test_en_125;
   reg [THREE_BIT-1:0]      test_en_125;
...
parameter                    A_BUS_NUMBER = 6;
parameter                    D_BUS_NUMBER = 16;
parameter                    INJ_NUMBER = 6;
parameter                    BANKS_NUMBER = 2;
parameter                    REG_BITS = 12;
parameter                    REG_NUMBER = 9;
parameter                    SPLIT_NUMBER = 3;
parameter                    INT_REG_NUM = 7;
parameter                    STATE_VAR_NUM = 4;
parameter                    ST_REG_NUM = 11;
parameter                    GLOB_STATE_BITS = 7;
parameter                    TWENTYONE_BIT = 21;
parameter                    TWO_BIT = 2;
parameter                    THREE_BIT = 3;
parameter                    FOUR_BIT = 4;
parameter                    FIVE_BIT = 5;
parameter                    SEVEN_BIT = 7;
parameter                    EIGHT_BIT = 8;
parameter                    NINE_BIT = 9;
parameter                    TEN_BIT = 10;
parameter                    THIRTEEN_BIT = 13;
parameter [BANKS_NUMBER-1:0] S = 2'b00;
parameter [BANKS_NUMBER-1:0] CH = 2'b01;
parameter [BANKS_NUMBER-1:0] R = 2'b10;
parameter [BANKS_NUMBER-1:0] N = 2'b11;
parameter [BANKS_NUMBER-1:0] A = 2'b00;
parameter [BANKS_NUMBER-1:0] B = 2'b01;
parameter [BANKS_NUMBER-1:0] C = 2'b10;
